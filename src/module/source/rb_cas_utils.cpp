 ...
 * @brief 该文件由程序自动生成, 核心业务逻辑部分
 *
 * @author jwzhou (zhou24388@163.com)
 * @version v0.5.0
 * @date 2023-06-07 17:27:55.454653
 *
...
/* 构造函数 */
CRbCASUtils::CRbCASUtils(RB_S32 s32W, RB_S32 s32H)
{
    /* 清理目标信息 */
    __clean_objs();
...
void CRbCASUtils::__init_para()
{
    ...
    m_s32ScaleH = 0;
    m_f32MaskScale = 0.5;
    m_enDangerSens = RB_CAS_DANGER_SENS_DEFAULT;
}

/* 初始化模型，加载模型参数 */
void CRbCASUtils::__init_model()
{
#ifdef WITH_NCNN
...
/* 设置尺寸以及利用尺寸初始化 */
void CRbCASUtils::__set_size(RB_S32 s32W, RB_S32 s32H)
{
    m_s32ImW = s32W;
    m_s32ImH = s32H;
...
int CRbCASUtils::config(const RB_CAS_Para_S *pstPara)
{
    if (NULL == pstPara)
    {
        return RB_FAILURE;
    ...
    {
    case RB_CAS_DANGER_SENS_LOW:
        m_dOverlapRatio *= LOW_SENS_RATIO;
        break;
    case RB_CAS_DANGER_SENS_HIGH:
...
// DONE
int CRbCASUtils::process(const RB_IMAGE_S* pstImage)
{
    int ret;
    ...
    std::cout<<"++++++  In CRbCASUtils: Network Process Image Time Span ++++++ "<< (dtEnd - dtStart) <<" ms"<<std::endl;
#endif // SHOW_DEBUG_TIME

    // 清空数据
    std::vector<RB_CAS_Ped_S>().swap(m_stResults.vecPedSet);
...
// 规则后处理
int CRbCASUtils::__post_process()
{
    // TODO: 
...
// 获得结果
int CRbCASUtils::get_results(RB_CAS_Result_S* pstResult)
{
    if (NULL == pstResult)
    {
        return RB_FAILURE;
    ...
    std::vector<RB_CAS_Ped_S>().swap(pstResult->vecPedSet);

    for (int i = 0; i < m_stResults.vecPedSet.size(); i++)
    {
        RB_CAS_Ped_S stCASPed = m_stResults.vecPedSet[i];
        pstResult->vecPedSet.push_back(stCASPed);
    }
